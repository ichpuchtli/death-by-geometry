...
## Boss Damage Feedback (per-hit, before the kill)

Shared, **data-driven** feedback so a multi-hit boss reads its damage while you unload into it (not just at the moment it shatters). Applies to Sierpinski tiers 0/1, Mandelbrot, and any future boss.

- **Flag:** `Enemy.bossFeedback` (set true on Sierpinski via `this.hp > 1`, and Mandelbrot). Only flagged, multi-HP enemies get this.
- **Damage model:** `Enemy.damageFraction` getter (0 pristine → 1 one hit from death). Shared diegetic-visual helpers on `Enemy`: `damageHeatColor(base, d)` (blends the body toward amber then white-hot as damage rises) and `damageShudder(time, d)` (render-only tremble offset that grows past 50% damage — **never** mutates `position`, so collision/AI stay stable).
- **Plumbing:** `collision.ts` pushes non-killing boss hits into optional `CollisionResult.bossHits[]` (contact point + bullet travel angle + `fracBefore`). `CombatSystem.processBossHits()` runs first inside `processKills()` (no `game.ts` change — folded into the existing `combat.processKills(result)` call; `bossHits` optional so hand-built debug results still work).
- **Subtle bite (per hit):** contact spark (`field.spawnBurst`, tinted by unit hue) + tiny grid dimple (`grid.applyImpulse`) + a soft **pitch-rising tick** `AudioManager.playBossHit(intensity)` (bandpass blip 420→940 Hz climbing with damage). Tick is rate-limited by `bossHitSoundCooldown` (`BOSS_HIT_SOUND_COOLDOWN_MS` 45ms) so rapid fire doesn't machine-gun it.
- **Milestone chunk:** crossing ¼/½/¾ damage (`BOSS_MILESTONE_FRACTIONS`) escalates to a heavier hit — bigger spark, grid punch, `camera.shake` (`BOSS_MILESTONE_SHAKE`), `BOSS_MILESTONE_HITSTOP` (30ms), and `AudioManager.playBossHitMilestone(intensity)` (low saturated thud + rising shard crack).
- **Diegetic body:** Sierpinski accumulates a permanent fracture line per survived hit (`crackAngles`, drawn hot inward from the rim) + heat tint + shudder; Mandelbrot folds the same heat tint + shudder into its existing render. **No HUD health bar** — HP is communicated purely on the body (chosen over a bar for the game's clutter-free aesthetic).
- Config in `config/combat.ts` (`BOSS_HIT_*`, `BOSS_MILESTONE_*`). Flow test: `tests/flows/100-boss-damage-feedback.yml`.
