...
- **Concave refraction (liquid-glass event horizon):** the hole's "light-bending" read is a **screen-space GLSL pass**, not geometry on the body. `Game.applyBlackHoleRefraction()` (in `render()`, between the scene FBO and bloom) runs `RefractionPass` (`renderer/refraction.ts`, reusing `shaders/liquid-glass.frag` concave variant) to lens the rendered scene behind each active hole with a **concave pinch** (grid + enemies bend/shrink inward = gravitational lensing) + a glassy specular highlight + fresnel rim. Fed to bloom via `BloomPass.sceneTexOverride`. Tuning: `BH_REFRACTION_PINCH` 2×, `BH_REFRACTION_LENS_RADIUS_MULT` 2.4 (× collisionRadius), caps 6 desktop / 3 mobile (`config/effects.ts`). Chosen + dialed in via the Liquid Glass Lab (`?liquid=1`). **The BlackHole body renders NO ring** — the earlier spectral photon ring (`renderGlassDiffraction`) was removed in favour of this. Note: dev labs (gallery/taxonomy/circle) render the hole body but don't run the refraction pass, so holes look plainer there.
