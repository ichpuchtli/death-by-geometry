...
- **Prominent spawn telegraph:** `BlackHole` overrides `renderSpawn()` (replaced the too-subtle generic gravity-well spawn — players kept drifting onto a hole as it formed). Draws an unmistakable "keep clear" warning that scales up with spawn progress: an **amber danger footprint** ring (grows to ~220px) with expanding radar-ping rings + a rotating dashed reticle, **imploding accretion rings**, and a growing dark core that resolves into the singularity. The grid gravity well (ramped by `spawnFactor` in `GravitySystem.updateGravityWells`) also deepens through the spawn, reinforcing it. Paired with the bassy `playBlackHoleSpawn()` sound (see audio skill). Flow test: `tests/flows/88-blackhole-spawn-telegraph.yml`.
