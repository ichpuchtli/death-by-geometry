...
**Threat Lab** (`?threat=1`): a playable BlackHole tuning arena (`web/src/threat-lab.ts`) that boots instead of the game. One BlackHole vs. the real player ship + a constant rhombus trickle that tracks the player. Keys **1-4** switch full threat presets (`THREAT_PRESETS`: CURRENT / DREADNOUGHT / CATACLYSM / SINGULARITY) varying HP, feed threshold, attract radius, enemy/player pull (+ "inescapable core" multiplier inside 40% of the radius), warning window, detonation payload (circles + shards), particle/shockwave/flash intensity, and supernova sound variant. **E** feeds the hole to critical, **Q** detonates instantly, **A** cycles the detonation sound independently of the preset (mix-and-match A/B), **R** resets. Faint rings visualize the attract radius + core. Supporting engine changes: `BlackHole.destabilizeDuration` instance field (config `SUPERNOVA_DESTABILIZE_MS = 1500`), `AudioManager.playSupernovaWarning(durationMs)`, and `AudioManager.playSupernovaVariant('classic'|'subdrop'|'doom'|'quake', absorbed)` — three new deep-bass detonation sounds. `window.threatLab` exposed. Flow test: `tests/flows/77-threat-lab.yml`.
