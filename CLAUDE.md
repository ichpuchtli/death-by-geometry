...
- BlackHole spawn grace + orbit-dot cleanup: **Complete** (BlackHoles now warp in over `SPAWN_DURATION_BLACKHOLE` = 3.0s (`config/spawner.ts`) instead of the 1.5s default — collision + gravity already skip `isSpawning` holes, so this is a long *harmless* telegraph that stops a lethal gravity well from materializing on top of the player; `SpawnSystem` ambush override uses `Math.max` so it never shortens it. Also removed the `dense` variant's orbit-dot particles (`drawFilledCircle` r≈2.5px, 8-seg) — sub-pixel specks that didn't register at gameplay zoom; swirl arms + infall streaks carry the accretion motion. Flow test: `tests/flows/81-blackhole-spawn-grace.yml`.)
