...
- Circle predictive tracking: **Complete** (supernova-ejected Circles were non-lethal — a same-speed pursuer aiming at the player's *current* position can never catch a fleeing player, and the `displacer` swirl-offset + `CIRCLE_FLOCK_PULL` cohesion made the flock orbit-past instead of striking. `CircleEnemy.update` now **leads the target** (`playerPos + playerVel × min(CIRCLE_LEAD_MAX_MS 550, dist/speed)`, first-order intercept, capped so a juke shakes it) and **commits on approach** — a `commit = min(1, dist/CIRCLE_COMMIT_RADIUS 220)` factor fades the displacer + flock pull to zero for a clean terminal strike. Constant-speed seek (never decelerates); `ENEMY_SPEED.circle` 0.35→0.38 ("slightly more lethal"). `playerVel` threaded through the enemy update loop in `game.ts` + `sim/headless-game.ts`. Config in `config/enemy.ts` (`CIRCLE_LEAD_MAX_MS`, `CIRCLE_COMMIT_RADIUS`). Flow test: `tests/flows/83-circle-predictive-tracking.yml`.)
